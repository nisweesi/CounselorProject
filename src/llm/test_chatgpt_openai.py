...
# Add the parent directory to sys.path to access the 'speech' folder
...
if not OPENAI_API_KEY:
    print("Error: OpenAI API key not found, please, set it as env variable")
    sys.exit(1)

# Initialize the SpeechDetector
...
messages=[
    {"role": "system", "content":f"""
        ...
        13. Be funny to a limit if the user needed to hear humurous
        Do not include any meta-text or explanations. Only provide the statement itself.
        """}
]

while True:
    user_input = speech_detector.detect_speech()
    ...
    try:
        ...
        if response.choices:
            ...
            if counselor_response:
                speech_model(counselor_response)
